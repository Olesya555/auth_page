...
Future<Album> fetchAlbum() async {
  final response = await http.get(Uri.parse('https://jsonplaceholder.typicode.com/albums/1')); // здесь интерент-адрес указан учебный
  ...
  if (response.statusCode == 200) { // у ответов response есть определённый статус , статусы разделяются на несколько групп
    ...
    return Album.fromJson(jsonDecode(response.body)); // если статус 200, т. е. был успешно получен ответ, то функция Album.fromJson
...
class Album { // создаём класс с нужными полями
  final int userId;
  final int id;
  final String title;

const Album({ // у есть конструктор класса (имя такое же), который принимает 3 поля
  ...
  factory Album.fromJson(Map<String, dynamic> json) { // конвертация идет из формата Json - fromJson(Map<String, dynamic> json)
...
late Future<Album> futureAlbum; // Описываем наш будущий futureAlbum, здесь есть ключевое слово late - отложенная загрузка
// используется стандартный механизм работы с отложенными операциями Future<Album>
  ...
  void initState() { // делаем загрузку данных  // т.е. инициализируем или первично настраиваем наш виджет состояния (_...)
    ...
    futureAlbum = fetchAlbum(); // происходит вызов метода загрузки наших данных из сети
  }
  ...
  return MaterialApp(
    ...
    home: Scaffold(
      ...
      body: Center(
        ...
        child: FutureBuilder<Album>(  // указываем тип данных <Album>
          future: futureAlbum, // указываем источник данных, откуда будут они загружены через метод fetchAlbum, который возвращает
          ...
          builder: (context, snapshot) { // метод builder, который принимает 2 параметра context и snapshot - он позволяет
            ...
            if (snapshot.hasData) {  // Обязательно проверяем, были ли загружены данные, через булевый параметр hasData (да/нет)
              return Text(snapshot.data!.title); // // возврат ответа, например, по полю наших данных .title.
